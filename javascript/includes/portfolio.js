...
{
	var downloadStr;
	if($('.mediaHolder1 img').attr('src'))
		downloadStr = $('.mediaHolder1 img').attr('src');
	else
		downloadStr = $('.mediaHolder2 img').attr('src');

	if($(window).width() <= 1279){
		
		var r = confirm("Ce fichier HD peut ralentir votre appareil et consommer beaucoup de bande passante. \n\nÊtes-vous sûr de vouloir continuer ? ");
		if (r == true) {
			window.open('download.html?file=' + downloadStr.substring(14).replace('main/', ''), '_parent');
		}
  	}
	else
		window.open('download.html?file=' + downloadStr.substring(14).replace('main/', ''), '_parent');
...
var firstGalleryImage = $("#firstGallery").val();
